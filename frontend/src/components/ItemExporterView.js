...
const ItemExporterView = ({ filters, selectedReport, loading }) => {
  const [products, setProducts] = useState([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [exporterData, setExporterData] = useState(null);
  const [allExportersData, setAllExportersData] = useState(null);
  const [expandedExporters, setExpandedExporters] = useState({});
  ...
  const productListId = "item-exporter-product-list";
  ...
  // Fetch product list
  useEffect(() => {
    if (selectedReport !== "item-exporter" || loading) return;
  ...
  useEffect(() => {
    setCurrentPage(1);
    setAllItemsPage(1);
    setExpandedExporters({});
  }, [queryBaseKey]);

  // Fetch all exporters or search by product
  useEffect(() => {
    if (selectedReport !== "item-exporter") return;
    ...
    if (cached) {
      if (cached.mode === "single") {
        setExporterData(cached.payload);
        setAllExportersData(null);
        setShowingAllItems(false);
      } else {
        setAllExportersData(cached.payload);
        setExporterData(null);
    ...
    const fetchExporters = async () => {
      ...
      try {
        if (normalizedSearch) {
          const res = await apiService.aiItemExporter({
            ...normalizedFilters,
          ...
          for (const product of pageProducts) {
            try {
              ...
              if (res.exporters && res.exporters.length > 0) {
                allData.push({
                  product,
                  exporters: res.exporters,
                  total: res.total_exporters,
                });
              }
            } catch (e) {
              console.error(`Failed to fetch exporters for ${product}:`, e);
            }
          }
          ...
          setAllExportersData(allData);
          setShowingAllItems(true);
        }
      } catch (err) {
        if (requestId === dataRequestRef.current) {
          setError(err.message || "Failed to fetch exporter data");
        }
      } finally {
        if (requestId === dataRequestRef.current) {
          setViewLoading(false);
  ...
  useEffect(() => {
    ...
    const fetchMoreExporters = async () => {
      try {
        const res = await apiService.aiItemExporter({
          ...normalizedFilters,
          product: searchQuery.trim(),
  ...
  const pagedAllExportersData = allExportersData || [];
  ...
  if (viewLoading && !exporterData && !allExportersData) {
    return (
  ...
  return (
    <div className="ie-root" style={{ marginBottom: "2rem" }}>
      <h2
        style={{ marginTop: "2rem", marginBottom: "1.5rem", color: "#0f172a" }}
      >
        Item {`>`} Exporter Hierarchy
      </h2>
        ...
        >
          <div style={{ padding: "1.5rem", borderBottom: "1px solid #e2e8f0" }}>
            ...
            <p style={{ color: "#64748b", fontSize: "0.9em" }}>
              Found {exporterData.total_exporters} exporters
            </p>
          </div>

          {/* Exporters List */}
          <div style={{ padding: "1.5rem" }}>
            {(exporterData.exporters || []).length === 0 ? (
              <p style={{ color: "#64748b", padding: "1rem" }}>
                No exporters found for this item.
              </p>
            ) : (
              (exporterData.exporters || []).map((exporter, idx) => (
                <div
                  key={idx}
                  style={{
                    marginBottom: "1.5rem",
                ...
                >
                  {/* Exporter Row */}
                  <div
                    onClick={() => toggleExporter(exporter.exporter)}
                    style={{
                      padding: "1rem",
                  ...
                  >
                    <div>
                      ...
                      >
                        {exporter.exporter_country} • {exporter.client_count}{" "}
                        clients
                      </div>
                    </div>
                    <span
                      style={{
                        ...
                        color: "#0f172a",
                        fontWeight: 600,
                        transform: expandedExporters[exporter.exporter]
                          ? "rotate(180deg)"
                          : "rotate(0deg)",
                  ...
                  {/* Partners Table (Expandable) */}
                  {expandedExporters[exporter.exporter] && (
                    <div
                    ...
                    >
                      ...
                      >
                        ...
                        <tbody>
                          {(exporter.clients || []).map((client, cidx) => (
                            <tr key={cidx} style={{ borderBottom: "1px solid #f1f5f9" }}>
                              ...
                              >
                                {client.quantity_kg.toLocaleString()}
                              </td>
                            </tr>
                          ))}
                        </tbody>
          ...
          {exporterData.total_exporters > pageSize && (
            <div
              style={{
                padding: "1.5rem",
            ...
            >
              ...
              <button
                ...
                disabled={
                  currentPage * pageSize >= exporterData.total_exporters
                }
                style={{
                  padding: "0.5rem 1rem",
                  borderRadius: "6px",
      ...
      {showingAllItems && allExportersData && (
        <div style={{ display: "grid", gap: "2rem" }}>
          {pagedAllExportersData.map((itemData, itemIdx) => (
            <div
            ...
            >
              ...
              >
                ...
                <p style={{ color: "#64748b", fontSize: "0.9em" }}>
                  {itemData.total} exporters
                </p>
              </div>

              <div style={{ padding: "1.5rem" }}>
                {(itemData.exporters || []).map((exporter, expIdx) => (
                  <div
                    key={expIdx}
                    style={{
                      marginBottom: "1rem",
                      paddingBottom: "1rem",
                      borderBottom:
                        expIdx < (itemData.exporters || []).length - 1
                          ? "1px solid #f1f5f9"
                          : "none",
                    }}
                  >
                    <div
                      onClick={() =>
                        toggleExporter(
                          `${itemData.product}-${exporter.exporter}`
                        )
                      }
                      style={{
                        cursor: "pointer",
          ...
          {(!allExportersData || allExportersData.length === 0) && (
      ...
      {!showingAllItems && !exporterData && (
        <p style={{ color: "#64748b", padding: "2rem", textAlign: "center" }}>
          Enter an item name to search and view exporters.
        </p>
      )}
    </div>
  );
...
export default ItemExporterView;